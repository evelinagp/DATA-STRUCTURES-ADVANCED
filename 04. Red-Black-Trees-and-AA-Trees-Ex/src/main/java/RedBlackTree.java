...
public class RedBlackTree<Key extends Comparable<Key>, Value> {
    ...
    public Value get(Key key) {
        if (key == null) {
            throw new IllegalArgumentException("Key can not be null in get() call!");
        }
    ...
    public void put(Key key, Value val) {
        if (key == null) {
            throw new IllegalArgumentException("Key can not be null in put() call!");
        }
    ...
    private int height(Node x) {
        ...
        int maxHeight = Math.max(height(x.left), height(x.right)) + 1;
        return maxHeight;
    }
